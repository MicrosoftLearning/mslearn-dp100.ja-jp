...
2. Azure Machine Learning スタジオで、 **[データセット]** ページを表示します。 データセットは、Azure ML 内で使用する予定の特定のデータ ファイルまたはテーブルを表します。
...
2. 次の設定を使用して、新しい自動 ML 実行を作成します。
    ...
    - **[Task type and settings]\(タスクの種類と設定\)**:
        ...
        - **[追加の構成設定を表示する]** を選択して、 **[追加の構成]** を開きます。
            - **プライマリ メトリック**: **AUC_Weighted** を選択します *"(このメトリックについては後で詳しく説明します)"*
